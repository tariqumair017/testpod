...
router.get("/add-test", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => {  
    res.render("Admin/AddTest");
}));
  
//Admin: Add Questions
router.post("/add-test", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => { 
  ...
  router.get("/manage-quiz", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => { 
    const data = await QuizModel.find({}); 
    res.render("Admin/ManageQuiz", { data });
  }));
  
  //Admin - Edit Test Name
  router.put("/manage-quiz/:id", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => { 
  ...
  router.delete("/manage-quiz/:id", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => { 
    const { id } = req.params;
  ...
  router.get("/manage-quiz/:id/all-quiz", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => {
    const data = await QuizModel.findById(req.params.id);
    if(!data)
    {
      req.flash("error", "Test Not Found");
      return res.redirect("/manage-quiz");
    }
    res.render("Admin/AllQuiz", { data });
  }));
  
  // Admin: Add new Question in Test
  router.post('/manage-quiz/:id/new', connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => { 
  ...
  // Admin: Edit Question
  router.get('/all-quiz/:id/edit', connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => { 
    const data = await QuizModel.findById(req.params.id);
    res.send(data);  
  }));
  
  //Admin: Update Question
  router.put("/all-quiz/:cid/:pid", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => {    
  ...
  router.delete("/all-quiz/:pid/:cid", connectEnsureLogin.ensureLoggedIn("/login"), asyncHandler(async (req, res, next) => {  
