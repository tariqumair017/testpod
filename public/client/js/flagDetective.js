  ...
  function showFlagDetectiveGame(index) {
    if (que_count <= flagDetective.length) {
      ...
      let detect_flag_image =
        `<span style="border-radius:7px;width:100%;height:250px;display:flex;justify-content:center;margin-right:5px; border: 2px solid #f9f9f9;padding:10px"><img class="border" src=${flagDetective[index].flagImage}
     alt="img"></span>` || `<div class="loading-skeleton"></div>`;
      ...
      detective_level.innerHTML =
        (flagDetective[index]?.gameLevel == "0" && "Easy") ||
        (flagDetective[index]?.gameLevel == "1" && "Normal") ||
        (flagDetective[index]?.gameLevel == "2" && "Hard") ||
        (flagDetective[index]?.gameLevel == "3" && "Extream");
