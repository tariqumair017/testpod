...
fetch('/admin/flag-detective-game/data-of-allFlags')
        .then(res => res.json())
        .then((json) => {   
            var _html = '';
...
document.getElementById("flagDetectiveID").addEventListener("change", async (event) => {
    ...
    const response = await fetch(`/admin/flag-detective-game/all-flags-data/country/${event.target.value}`);
...
add_new_form.onclick = async () => {
  ...
  if(region_input != '')
  {
    const allCountries = await fetch(`/admin/flag-detective-game/all-flags-data/country/${region_input}`);
...
document.getElementById("allCountries").addEventListener("change", async (event) => {   
    const response = await fetch(`/admin/flag-detective-game/all-flags-data/country-for-flag/${event.target.value}`);
    const data = await response.json(); 
...
async function flagDetectiveSelectedCountry(e)
    { 
      ...
      const response = await fetch(`/admin/flag-detective-game/all-flags-data/country-for-flag/${countryName}`);
      const data = await response.json();
